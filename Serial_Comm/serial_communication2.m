...
data_temp = getappdata(handles.figure1, 'data_temp');
...
global t;
global data_val1;
if(strcmp(data_temp(1,1:end),'55') && strcmp(data_temp(2,1:end),'AA') && strcmp(data_temp(end,1:end),'FF'))
    FF = 0;
        % axes(handles.plotAD);
 t(i) =  0.05*i;
 %t =[t 0.05*i]
 i=i+1;
 val = [val str2num(data_temp(3,1:end))];
  data_val1 = [data_val1 str2num(data_temp(4,1:end))];
 plot(handles.plotAD,val);
 plot(handles.axes2,data_val1);
end
...
if n
    ...
    setappdata(handles.figure1, 'data_temp', data_temp);
end
